...
import { athlete as mockAthleteData, sportMatches } from '../../../../src/data/mockData'
import { ShieldCheck, ClipboardCheck, Check, TrendingUp } from 'lucide-react'
import { getScoutAthleteProfile } from '../../../../src/lib/api/scouts'
...
export default function ScoutAthleteProfile({ params }) {
  ...
  useEffect(() => {
    async function load() {
      try {
        const res = await getScoutAthleteProfile(params.id)
        setProfile(res)
      } catch (err) {
        console.error(err)
      } finally {
  ...
  if (loading) return <Desk title="Loading profile..."><div className="p-8 text-center text-muted">Loading...</div></Desk>
  if (!profile) return <Desk title="Athlete not found"><div className="p-8 text-center text-muted">Not found or unauthorized</div></Desk>

  const a = profile.athlete
  const verification = profile.verification_score || 0
  
  // We'll use mock data for attributes since we don't have DNA dimensions fetched in this simple API yet
  const attributes = mockAthleteData.attributes

  return (
    <Desk title={a.name || a.id} subtitle={`${profile.sport_potential ? 'Multiple Sports' : 'Unassigned'} · ${a.location} · Age ${a.age}`}>
          ...
          <FadeIn>
            <div className="card p-6">
              <div className="flex justify-between">
                <div>
                  <p className="text-[10px] font-bold tracking-wider text-muted">SCOUT REVIEW SCORE</p>
                  <p className="mt-1 text-[44px] font-extrabold leading-none">
                    {profile.scout_review_score || 0}<span className="text-lg text-muted">/100</span>
                  </p>
                </div>
                <span className="self-start flex items-center gap-1 rounded-full bg-green/30 px-3 py-1.5 text-xs font-bold text-green-deep">
                  <ShieldCheck size={13} /> {verification}% Verified
                </span>
              </div>
