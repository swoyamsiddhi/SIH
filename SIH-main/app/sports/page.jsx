...
export default function Sports() {
  const [sportMatches, setSportMatches] = useState([])
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    async function loadPotential() {
      const matches = await api.athletes.getSportPotential()
      setSportMatches(matches)
      setLoading(false)
    }
    loadPotential()
  }, [])

  if (loading) return <div className="p-10 text-center text-muted">Loading sport matches...</div>

  if (sportMatches.length === 0) {
    return (
      <main className="phone nav-safe px-5">
        <Header title="Sport potential" back />
        <div className="p-10 text-center text-muted">Insufficient data to recommend sports. Please complete assessments.</div>
        <BottomNav />
      </main>
    )
  }
