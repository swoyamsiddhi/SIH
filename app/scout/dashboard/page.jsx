...
import { scoutStats, scoutAthletes } from '../../../src/data/mockData'
  ...
  return (
    <Desk title="Talent overview" subtitle="Your verified athlete pipeline">
      ...
      <section className="mt-8 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <KpiCard label="Athletes assessed" value={scoutStats.athletesAssessed} tone="bg-blue-pale" chip="Updated today" icon={Users} />
        <KpiCard label="High potential" value={scoutStats.highPotential} tone="bg-green/30" chip="Top 2.8%" icon={Sparkles} />
        <KpiCard label="New alerts" value={scoutStats.newAlerts} tone="bg-coral-pale" chip="This week" icon={Award} />
        <KpiCard label="Upcoming trials" value={scoutStats.upcomingEvents} tone="bg-amber-pale" chip="Next 30 days" icon={Search} />
      </section>
            ...
            {scoutAthletes.slice(0, 6).map(x => (
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <b className="text-sm">{x.id}</b>
                    <p className="mt-0.5 text-xs text-muted">{x.sport} · {x.location} · Age {x.age}</p>
                  </div>
                </div>
                <div className="text-right">
                  <b className="text-lg">{x.potential}%</b>
                  <p className="text-[10px] font-bold text-green-deep">↑ +{x.growth}% growth</p>
                </div>
              </Link>
            ))}
          </div>
